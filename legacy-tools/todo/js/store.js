...
export function addTask(draft) {
    ...
    if (!trimmedTitle) {
        throw new Error('addTask: title is required and cannot be empty/whitespace');
    }
    if (!draft.groupId) {
        throw new Error('addTask: groupId is required');
    }
    const id = uid('t');
    patch((s) => {
        const sameGroup = s.tasks.filter((t) => t.groupId === draft.groupId);
...
export function addSubtask(taskId, title) {
    ...
    if (!trimmedTitle) {
        throw new Error('addSubtask: title is required and cannot be empty/whitespace');
    }
    const id = uid('s');
    patch((s) => ({
        tasks: s.tasks.map((t) => {
            if (t.id !== taskId) return t;
            const subtasks = [...(t.subtasks || []), { id, title: trimmedTitle, done: false }];
            return { ...t, subtasks };
        }),
...
// 重排序：传入完整的新数组（同级）
// 实际重排数组顺序，使调用 getState() 后 tasks 的顺序与新顺序一致
export function reorderTasks(groupId, newOrderedIds) {
    patch((s) => {
        const orderMap = new Map(newOrderedIds.map((id, i) => [id, i]));
        // 收集同组任务并按 newOrderedIds 重排
        const inGroup = s.tasks.filter((t) => t.groupId === groupId);
        const reorderedInGroup = newOrderedIds
            .map((id) => inGroup.find((t) => t.id === id))
            .filter(Boolean)
            .map((t, i) => ({ ...t, order: i }));
        // 不在新数组中的同组任务追加到末尾
        const inNewSet = new Set(newOrderedIds);
        const extrasInGroup = inGroup
            .filter((t) => !inNewSet.has(t.id))
            .map((t, i) => ({ ...t, order: reorderedInGroup.length + i }));
        const newInGroup = [...reorderedInGroup, ...extrasInGroup];
        const others = s.tasks.filter((t) => t.groupId !== groupId);
